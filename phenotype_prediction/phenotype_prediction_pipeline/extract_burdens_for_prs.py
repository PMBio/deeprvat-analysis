...
def combine_significant(sig_file, min_discoveries, out_file):
    ...
    all_discoveries = all_discoveries.query("Method != 'DeepRVAT wo baseline'")
    
    existing_combinations = {(p, m) for p in phenotypes for m in methods_to_keep}
    disc_counts = all_discoveries.query('Method in @methods_to_keep')\
        .groupby(['phenotype', 'Method']).size()\
        .reset_index()\
        .rename(columns = {0: 'discoveries'})
    logger.info(f'Significant discoveries {disc_counts}')
    logger.info(f'Requiring at least {min_discoveries} discoveries. Taking genes with lowest pval of not enough genes are significant')

    suff_counts = disc_counts.query('discoveries >= @min_discoveries')

    existing_data_set = set(tuple(row) for row in suff_counts[['phenotype', 'Method']].to_numpy())
    ...
    missing_combinations = pd.DataFrame(missing_combinations, columns = ['phenotype', 'Method'])
    ...
    if min_discoveries == 0:
        ...
        logger.info('Caution: The following phenotype/method combinations have zero discoveries')
        logger.info(missing_combinations)
    else:
        all_discoveries = suff_counts[['phenotype', 'Method']].merge(all_discoveries, how = 'left')
        assert len(all_discoveries[['phenotype', 'Method']].drop_duplicates()) == len(suff_counts)

        for pheno in missing_combinations['phenotype'].unique():
            logger.info(f'Adding missing discoveries for phenotype {pheno}')
            missing_methods = missing_combinations.query('phenotype == @pheno')['Method'].unique()
            all_pvals = pd.read_parquet(f"{os.path.dirname(sig_file_dict[pheno])}/all_results.parquet")
            top_associations = all_pvals.query('Method in @missing_methods')\
        ...
        assert len(all_discoveries[['phenotype', 'Method']].drop_duplicates()) == len(existing_combinations)
