...
min_genes = [
    1
]  # minimum number of top n genes if there are zero discoveries for a trait. Will take the top min_genes according to p-values then.
# If there are significant genes, all significant genes will be considered irrespective of min_genes.
max_sig = max(min_genes)


rule all_phenotype_prediction:
    input:
        expand(
            "phenotype_prediction/models/linear_models/top_{min_n_genes}/plotting_data/replication_in_extreme/ranked_df_combined_{outlier_mode}_{extreme_quantile}_{phenotype_suffix}_{fdr}.Rds",
        ...
        expand(
            "phenotype_prediction/models/linear_models/top_{min_n_genes}/plotting_data/enrichment_prs_vs_zscore_{quantile}_{phenotype_suffix}_{fdr}.Rds",
            phenotype_suffix=phenotype_suffixes,
            quantile=[0.99, 0.999],
            min_n_genes=min_genes,
            fdr=fdrs,
        ...
        expand(
            "phenotype_prediction/models/linear_models/top_{min_n_genes}/plotting_data/all_recomputed_metrics_test_{phenotype_suffix}_{fdr}.Rds",
            phenotype_suffix=phenotype_suffixes,
            min_n_genes=min_genes,
            fdr=fdrs,
        ),
...
rule replication_in_extremes_plot:
    conda:
        "r-env"
    input:
        "phenotype_prediction/models/linear_models/top_{min_n_genes}/plotting_data/plot_df_list_sub_{phenotype_suffix}_{fdr}.finished",
    output:
        "phenotype_prediction/models/linear_models/top_{min_n_genes}/plotting_data/replication_in_extreme/ranked_df_combined_{outlier_mode}_{extreme_quantile}_{phenotype_suffix}_{fdr}.Rds",
    params:
        fdr="{fdr}",
        phenotype_suffix="{phenotype_suffix}",
        linear_model_res_path="phenotype_prediction/models/linear_models/top_{min_n_genes}/",
...
rule split_plot_df_list:
    conda:
        "r-env"
    input:
        "phenotype_prediction/models/linear_models/top_{min_n_genes}/plotting_data/plot_df_list_{phenotype_suffix}_{fdr}.Rds",
    output:
        "phenotype_prediction/models/linear_models/top_{min_n_genes}/plotting_data/plot_df_list_sub_{phenotype_suffix}_{fdr}.finished",
    params:
        fdr="{fdr}",
...
rule make_plot_df_list_data_linear_model:
    ...
    input:
        expand(
            "phenotype_prediction/models/linear_models/top_{{min_n_genes}}/{phenotype}/{{phenotype_suffix}}_fdr-{{fdr}}.Rds",
            phenotype=phenotypes,
        ),
    output:
        "phenotype_prediction/models/linear_models/top_{min_n_genes}/plotting_data/plot_df_list_{phenotype_suffix}_{fdr}.Rds",
    params:
        input_res_path="phenotype_prediction/models/linear_models/top_{min_n_genes}/",
        out_dir="phenotype_prediction/models/linear_models/top_{min_n_genes}/plotting_data/",
        fdr="{fdr}",
        phenotype_suffix="{phenotype_suffix}",
        phenotypes=phenotypes,
    resources:
...
rule prep_metrics_logistic_model:
    ...
    input:
        expand(
            "phenotype_prediction/models/logistic_models/top_{{min_n_genes}}/{phenotype}/{{phenotype_suffix}}_{top_bottom_q}-{top_q}_fdr-{{fdr}}.Rds",
            phenotype=phenotypes,
            top_q=top_quantiles,
            top_bottom_q=top_bottom_quantiles,
        ),
    output:
        metrics="phenotype_prediction/models/logistic_models/top_{min_n_genes}/plotting_data/combined_metrics_{phenotype_suffix}_{fdr}.Rds",
    params:
        input_res_path="phenotype_prediction/models/logistic_models/top_{min_n_genes}/",
        out_dir="phenotype_prediction/models/logistic_models/top_{min_n_genes}/plotting_data/",
        fdr="{fdr}",
        phenotype_suffix="{phenotype_suffix}",
        phenotypes=phenotypes,
...
rule fit_logistic_regression_model_fdr:
    ...
    input:
        expand(
            "phenotype_prediction/r_data/top_{{min_n_genes}}/{{phenotype}}/cv_split{cvsplit}/data.finished",
    ...
    resources:
        ...
        load=16000,
    params:
        model_out_file="phenotype_prediction/models/logistic_models/top_{min_n_genes}/{phenotype}/{phenotype_suffix}_{top_bottom_q}-{top_q}_fdr-{fdr}.Rds",
        phenotype="{phenotype}",
        phenotype_suffix="{phenotype_suffix}",
        top_q="{top_q}",
        regression_data_input_path="phenotype_prediction/r_data/top_{min_n_genes}/{phenotype}/",
...
rule fit_linear_regression_model_fdr:
    ...
    resources:
        ...
        load=16000,
    params:
        model_out_file="phenotype_prediction/models/linear_models/top_{min_n_genes}/{phenotype}/{phenotype_suffix}_fdr-{fdr}.Rds",
        phenotype="{phenotype}",
        phenotype_suffix="{phenotype_suffix}",
        regression_data_input_path="phenotype_prediction/r_data/top_{min_n_genes}/{phenotype}/",
        config=regression_config["r_config"],
...
rule prep_data_for_r:
    input:
        ...
        discoveries="cv_split{cvsplit}/deeprvat/eval/all_significant_top{min_n_genes}.parquet",
    output:
        "phenotype_prediction/r_data/top_{min_n_genes}/{phenotype}/cv_split{cvsplit}/data.finished",
    params:
        dataset_dir="cv_split{cvsplit}/deeprvat/{phenotype}/deeprvat/",
        burdens_dir="phenotype_prediction/burdens/cv_split{cvsplit}",
        out_dir="phenotype_prediction/r_data/top_{min_n_genes}/{phenotype}/cv_split{cvsplit}",
...
rule extract_burdens:
    input:
        discoveries=f"cv_split{{cvsplit}}/deeprvat/eval/all_significant_top{max_sig}.parquet",
...
rule all_discoveries:
    input:
        expand(
            "cv_split{cv_split}/deeprvat/eval/all_significant_top{min_n_genes}.parquet",
...
rule combine_discoveries:
    ...
    shell:
        ...
        py + "extract_burdens_for_prs.py combine-significant "
                    " {params.sig_files} "
                    " --min-discoveries {wildcards.min_n_genes} "
                    " {output}"
                ),
            ]
        )
