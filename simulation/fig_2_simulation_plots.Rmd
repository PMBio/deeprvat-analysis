...
plot_data_path = "~/home/analysis/paper_plots/plot_data/simulation"

plot_out_path = './plots'
simulation_exp_dir = '~/ukbb/experiments/revision_1_experiment_links/simulations/'  #this should be the same path as `sim_res_dir` in config_eval of simulation/simulation_analysis
...
```{r}
p_maf_filter = ggplot(avg_metrics_maf_filter_combined %>% filter(metric == "Power_bf" & quantile == 0.5 & test %in% tests_to_keep), 
    aes(y=mean, x='', fill = test)) +  #, color = 'rare_maf')) + 
    # geom_bar(stat="identity")
...
ggsave(file.path(plot_out_path, 'fig_2_supp_calibration.png') , p, 
