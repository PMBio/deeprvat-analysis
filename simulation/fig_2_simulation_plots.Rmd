...
plot_data_path = 'X' #this should be the same path as `r_plot_data_path` in config_eval of simulation/simulation_analysis

plot_out_path = './plots'
simulation_exp_dir = 'X'  #this should be the same path as `sim_res_dir` in config_eval of simulation/simulation_analysis
...
```{r}
p_maf_filter = ggplot(avg_metrics_maf_filter_combined %>% filter(metric == "Power" & quantile == 0.5 & test %in% tests_to_keep), 
    aes(y=mean, x='', fill = test)) +  #, color = 'rare_maf')) + 
    # geom_bar(stat="identity")
    geom_col(position = position_dodge(), width = 0.9, color = 'darkgrey') + 
    geom_errorbar(aes(ymin = mean-std, ymax = mean+std), position = position_dodge(0.9),
        stat = 'identity', width = 0.4) +
    scale_fill_manual(values=colors_methods, guide = 'none') +
    facet_grid(cols = vars(rev_assoc_maf), rows = vars(group), labeller = labeller(.rows = label_both, .cols = label_both)) + 
...
p_prop = ggplot(avg_vary_prop_causal %>% filter(metric == 'Power' & test %in% tests_to_keep
...
deeprvat_legend_vert =  ggplot(avg_vary_prop_causal %>% filter(metric == 'Power' & test %in% tests_to_keep
        & prop_causal_variants %in%  variant_proportions), 
    aes(y= mean, x=factor(prop_causal_variants), fill = test)) +  
    geom_col(stat = 'identity', position = position_dodge(), width = 0.9, color = 'darkgrey') +
    geom_errorbar(aes(ymin = mean-std, ymax = mean+std), position = position_dodge(width = 0.9),
...
# FDR plot
```{r}
fdr_avg_metrics =  read_parquet(paste(plot_data_path, 'fdr_avg_metrics.parquet', sep = '/'))
fdr_avg_metrics = fdr_avg_metrics %>% mutate(default_fdr = ifelse(`expected FDR` == 0.05, TRUE, FALSE)) 

fdr_avg_metrics
```

```{r}
p_fdr_deeprvat_default_fdr = (ggplot(fdr_avg_metrics %>% filter(metric == "FDR" & test %in% c("DeepRVAT_wo_baseline") & prop_causal_variants == 0.2 & 
            `expected FDR` < 0.2), 
        aes(x=`expected FDR`, y= mean, color = default_fdr)) #, color = 'rare_maf'))
        + geom_point()
        + geom_errorbar(aes(x=`expected FDR`, ymin = mean-std, ymax = mean+std), width = 0.002) 
        + scale_color_manual(values = c('darkred', 'black'), limits = c(TRUE, FALSE), guide = FALSE)
        # + facet_grid('test ~ prop_causal_variants')
        + labs(x = 'expected FDR', y = 'empirical FDR') 
        + theme_classic() 
        + theme(aspect.ratio=1) 
        # + scale_x_continuous(limits = [0, 0.17])
        # + scale_y_continuous(limits = [0, 0.17])
        + geom_abline(intercept=0, # set the y-intercept value
                  slope=1      # set the slope value
                 )
        )
p_fdr_deeprvat_default_fdr
#ggsave(paste(plot_out_path, 'p_fdr_deeprvat_default_fdr.png', sep = '/'),p_fdr_deeprvat_default_fdr, width = 5, height = 6)

```

```{r}
#name DeepRVAT_wo_baseline as DeepRVAT and drop DeepRVAT with baseline

plot_data = fdr_avg_metrics %>% filter(test != 'DeepRVAT') 
plot_data[['test']][plot_data[['test']] == 'DeepRVAT_wo_baseline'] = 'DeepRVAT'

p_fdr_default_p_causal = (ggplot(plot_data %>% filter(metric == "FDR" & prop_causal_variants == 0.2 &
            `expected FDR` < 0.2), 
        aes(x=`expected FDR`, y= mean, color = default_fdr)) #, color = 'rare_maf'))
        + geom_point()
        + geom_errorbar(aes(x=`expected FDR`, ymin = mean-std, ymax = mean+std), width = 0.002) 
...
p_fdr_default_p_causal

# #ggsave(paste(plot_out_path, 'p_fdr_default_p_causal.png', sep = '/'),p_fdr_default_p_causal, width = 10, height = 5)
...
```{r}
p_fdr_alL_configs = (ggplot(fdr_avg_metrics %>% filter(metric == "FDR" & 
            `expected FDR` < 0.2), 
        aes(x=`expected FDR`, y= mean,  color = default_fdr)) #, color = 'rare_maf'))
        + geom_point()
        + geom_errorbar(aes(x=`expected FDR`, ymin = mean-std, ymax = mean+std), width = 0.002) 
...
#ggsave(paste(plot_out_path, 'p_fdr_alL_configs.png', sep = '/'),p_fdr_alL_configs, width = 10, height = 15)
```
# QQ-plots
...
lambda_df = all_pvals  %>% 
  ...
  # rename('lambda' = 'pval') %>%
  mutate(lambda_str = paste('λ', lambda, sep = '='))
lambda_df
```
...
qqplot_causal_non_causal = function(df, plot_title, lambda_df = NULL, point_color = 'black', color_by_causal = FALSE){
  ...
  if (!is.null(lambda_df)) {
    ...
    plot <- plot +
      geom_text(data = lambda_df,
                aes(x = x_text, y = y_text, label = lambda_str), color = 'black', alpha = 1)
  }
  return(plot)
}
...
for (this_method in levels(all_pvals$method)){
  ...
  this_lambda_df = lambda_df %>%  filter(`repeat` == 'sim_repeat_0')  %>% filter(method == this_method)
...
for (this_method in levels(all_pvals$method)){
  ...
  this_lambda_df = lambda_df %>%  filter(`repeat` == 'sim_repeat_0')  %>% filter(method == this_method)
  # title = ifelse(this_method %in% names(baseline_renamer), baseline_renamer[[this_method]], this_method)
  p = qqplot_causal_non_causal(plot_df, plot_title = this_method, this_lambda_df, colors_methods[[this_method]])
  p_list = append(p_list, list(p))
}
qq_arange_causal_split = ggarrange(plotlist = p_list, ncol = 2, nrow = 3) +
  theme(plot.margin = margin(1,1,0,1, "cm")) 
qq_arange_causal_split
...
legend_qq = g_legend(qq_legend_plot)
...
gt = grid.arrange(p_grid_maf_filter,
             ggarrange(plotlist = p_list_causal_split, ncol = 2, nrow = 3),
              legend_intervals, legend_qq, legend_methods, 
          nrow = 4, ncol = 8,
          layout_matrix = lay, heights = c(0.05,1, 0.1, 0.1),
          widths = c(0.2,1.2, 1.2, 0.2,0.8,0.8,1,1))
p = as_ggplot(gt) +                                # transform to a ggplot
  draw_plot_label(label = c("a", "b"), size = plot_width/default_plot_width * label_default_size,
                  x = c(0, 0.42), y = c(1, 1)) # Add labels

ggsave(file.path(plot_out_path, 'fig_2_v2.png'), p,
       width = plot_width,
       height = 20,
       unit = 'cm',
       dpi = 600)

# ggsave(file.path(plot_out_path, 'fig_2_v2.pdf'), p,
#        width = plot_width,
#        height = 18,
#        unit = 'cm',
#        dpi = 600)
...
ggsave(file.path(plot_out_path, 'fig_2.png'), p, 
       width = plot_width, 
       height = 18,
       unit = 'cm',
       dpi = 600)

ggsave(file.path(plot_out_path, 'fig_2.pdf'), p, 
       width = plot_width, 
       height = 18,
       unit = 'cm',
       dpi = 600)
...
```{r}
# all pvalues in one plot
qq_df_list = list()
for (this_method in levels(all_pvals$method)){
  print(this_method)
  df = all_pvals %>% filter(`repeat` == 'sim_repeat_0') %>% filter(method == this_method)
  df <- df %>% arrange(pval) 
  df['-log10pval_expected'] <- -log10(1:nrow(df) / nrow(df))
  ...
  qq_df_list = append(qq_df_list, list(df))
}
all_pvals_qq = do.call(rbind, qq_df_list)

line_color = 'black'
plot <- ggplot(all_pvals_qq, aes(x = `-log10pval_expected`, y = `-log10pval`, color = method)) +
...
# p = ggarrange(p_fdr_default_p_causal, box_plot_lambda, p_prop, p_replication, 
...
lay = rbind(c(NA,NA,NA, NA, NA,NA),
            c(1,1,1, NA, 2,2),
            c(3,3,4, NA, 5, 5))
deeprvat_legend = g_legend(deeprvat_legend_vert)

p = grid.arrange(p_fdr_default_p_causal, box_plot_lambda, p_prop, deeprvat_legend, p_replication,
          nrow = 3, ncol = 6,
          layout_matrix = lay, heights = c(0.1, 1.5,1),
          widths = c(1,1,1,0.2, 1,1))



p = as_ggplot(gt) +                                # transform to a ggplot
  draw_plot_label(label = c("a", "b", 'c', 'd'), size = 20,
                  x = c(0, 0.6, 0, 0.6), y = c(1,1, 0.45, 0.45)) # Add label
...
plot_width = 20
ggarrange(p_fdr_default_p_causal, 
          box_plot_lambda, 
          ncol = 2, nrow =1, widths = c(1.5, 1), labels = 'auto',
          font.label = list(size = plot_width/default_plot_width * label_default_size))
ggsave(file.path(plot_out_path, 'fig_2_supp_calibration.png') , width = plot_width, height = 15, unit = 'cm',
       dpi = 600)
ggsave(file.path(plot_out_path, 'fig_2_supp_calibration.pdf') , width = plot_width, height = 15, unit = 'cm',
       dpi = 600)

ggarrange( p_prop + labs(fill = '', color = ''), p_replication + labs(fill = '', color = ''), 
          ncol = 2, nrow =1,  labels = 'auto', common.legend = TRUE,  legend="bottom",
          font.label = list(size = plot_width/default_plot_width * label_default_size ))  
ggsave(file.path(plot_out_path, 'fig_2_supp_additional_simulations.png'), width = plot_width, height = 10, unit = 'cm',
       dpi = 600)
ggsave(file.path(plot_out_path, 'fig_2_supp_additional_simulations.pdf'), width = plot_width, height = 10, unit = 'cm',
       dpi = 600)
