...
class DeepSet(BaseModel):
    ...
    def __init__(
        self,
        config: dict,
        pheno:str, ## MODIFIED: NEW ARGUMENT FOR SHAP COMPABILITY.
        n_annotations: Dict[str, int],
        n_covariates: Dict[str, int],
        n_genes: Dict[str, int],
        phenotypes: List[str],
    ...
    # ORIGINAL FORWARD FUNCTION. Runs mutagenesis. 
    ...
    # MODIFIED FORWARD FUNCTION TO RUN SHAP.
    # input and covariates are needed to provided seperately 
    # Comment out below & un-comment original Forward above to run the model regularly 
    def forward(self, rare_variant_annotations, x_covariates): 
        ...
        x = self.gene_pheno[self.pheno](x) #.squeeze(dim=1)
