    ...
    for (pheno in phenotypes) {
        ...
        if (file.exists(this_res_file)) {
            ...
            if (file.exists(this_res_file)) {
                print(paste(this_res_file, "exists"))
                this_df = read_parquet(this_res_file) %>%
                  rename(EAC_filtered = "CAF_filtered")
                all_baseline_res = rbind(all_baseline_res, this_df)
            } else {
...
print(test_results %>%
    group_by(gene_id, across(all_of(pheno_col))) %>%
    summarise(n = n()) %>%
    ungroup() %>%
    arrange(n) %>%
    distinct(n))
if (!is.na(combine_pvals)) {
    test_results = aggPvalsToGene(test_results, combine_pvals, grouping_cols = c("gene_id", pheno_col)) %>%
        select(-pval_agg_method)
}
stopifnot(as.numeric(test_results %>%
    group_by(gene_id, across(all_of(pheno_col))) %>%
    summarise(n = n()) %>%
    ungroup() %>%
    arrange(n) %>%
...
replication_monti <- add_replication(test_results %>%
    rename(gene = gene_id), comparison, thresh = pval_thres, pheno_col = pheno_col) %>%
    ungroup() %>%
