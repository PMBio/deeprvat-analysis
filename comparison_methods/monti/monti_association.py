...
DEEPRIPE_COLS = [f"DeepRipe_plus_{rbp}" for rbp in rbps]
variant_weight_th_dict = {"sift_polyphen": 0.8, "spliceai": 0.1, "deepripe": 0.25}
...
# Functions for DeepRipe kernels copied from faatpipe pacakge
...
# # 1 for pLOF variants, else average of 1-SIFT and PolyPhen
# # For splicing variants, maximum of four scores
# # For binding RBP largest absolute value of variant effect predictions


# start: kernel definition functions
def get_weights(variant_ids, annotation_df, weight_cols, var_weight_function):
    ...
    elif var_weight_function == "spliceai":
        weight = get_splice_ai_weights(anno)
    elif var_weight_function == "deepripe":
        weight = get_deepripe_weights(anno)
    else:
        raise NotImplementedError(
...
def get_splice_ai_weights(anno):
    weight = anno[
        "SpliceAI_delta_score"
    ]  # this score is already the max of the 4 scores
    return weight.to_numpy()


def get_deepripe_weights(anno):
    weight = anno[DEEPRIPE_COLS].to_numpy()
    weight = np.max(np.abs(weight), axis=1)
    return weight
...
def get_anno(
    ...
    # dim = variants x annotations
    similarity_cols = list(set(dataset.rare_embedding.annotations) - set(weight_cols))
    A = annotation_df[similarity_cols].to_numpy()

    weights, plof_mask = get_weights(
        variant_ids, annotation_df, weight_cols, var_weight_function
    )

    S = None

    def cluster_variants_by_aa(annotation_df, variant_ids):
        anno = annotation_df.loc[variant_ids, :]
        # makes indicator matrix from amino acid assignments of variants
        # n_snp x n_amino_acids
        S = pd.get_dummies(anno["Protein_position"])
        # check that all variants are assigned to an AA or a dummy AA
        assert all(S.sum(axis=1) > 0)
        return S

    #
    if (
        ("missense" in var_type) & (test_type == "skat") & aa_collapse_missense
    ):  # this is the case if missense in var_type
        logger.info("Preparing matrix S for clustering variants by amino acid")
        S = cluster_variants_by_aa(annotation_df, variant_ids)

    if var_type == "deepripe":
        logger.info("Computing S matrix for deepripe")
        anno = annotation_df.loc[variant_ids, :]
        Q = anno[DEEPRIPE_COLS].to_numpy()
        Q = get_simil_from_anno(Q)
        R = get_simil_from_pos(np.array(anno["pos"]))
        S = np.multiply(Q, R)

    # cummac = mac_report.loc[temp_vids].Minor

    return weights, plof_mask, S
...
def test_gene(
    ...
    aa_collapse_missense = test_config.get("aa_collapse_missense", False)
    ...
    if "deepripe" in var_type:
        var_weight_function = "deepripe"
    if "spliceai" in var_type:
        var_weight_function = "spliceai"
    logger.info(f"Using function {var_weight_function} for variant weighting")

    # keep backwards compatibility
    weights, plof_mask, S = get_anno(
        G,
        variant_ids,
        annotation_df,
        weight_cols,
    ...
    if ((var_type == "missense_plof") | (var_type == "spliceai_plof")) & (
        test_type == "skat"
    ):
        # logger.info('Vtype is missens_plof, removing plof variants from kernel')
        pos = np.setdiff1d(pos, np.where(plof_mask)[0])
    if len(pos) > 0:
        G_f = G[:, pos]
        ...
        if test_config["center_genotype"]:
            ...
            G_f = center(G_f)
        weights_f = np.sqrt(weights[pos])
        ### here dot product of genotype matrix with weights
        if (S is not None) & (test_type == "skat"):
            # logger.info('Multiplying with S matrix')
            if "missense" in var_type:
                S = S.iloc[pos]
                # remove columns that are all 0 because variants were subset
                S = S.loc[:, (S != 0).any(axis=0)]
                # S is #variants x #clusters (different amino acids)
                # Multiply each column of S by the variant weights
                # C = np.array(S) * weights_f.reshape((-1, 1)) # this is the same as the matrix mult below
                C = np.array(S).transpose().dot(np.diag(weights_f, k=0)).transpose()
                # logger.info(f'Aggregating variants by AA, Number of clusters: {C.shape[1]}')
                GW = G_f.dot(C)
            if "deepripe" in var_type:
                S = S[np.ix_(pos, pos)]
                if G_f.shape[1] > 1:
                    ...
                    L, flag1 = np.eye(G_f.shape[1]), -1
                GW = G_f.dot(np.diag(weights_f, k=0)).dot(L)
        else:
            # logger.info('No matrix S provided. Multiplying genotypes with weights')
        ...
        if test_type == "skat":
            ...
            GW = GW
            if (var_type == "missense_plof") | (var_type == "spliceai_plof"):
                # #logger.info('Appending plof burden to kernel matrix')
                G_f_b = G[:, plof_mask]
                GW_burden = G_f_b.dot(np.diag(weights[plof_mask], k=0))
                collapse_method = test_config.get("collapse_method", "binary")
                GW_burden = collapse_burden(GW, method=collapse_method)
                GW = np.concatenate([GW, GW_burden], axis=1)
...
def update_config(
    ...
    if variant_type is not None:
        ...
        elif variant_type == "missense_plof":
            rare_threshold_config["missense_or_plof"] = "missense_or_plof == 1"
            config = _add_annotation_cols(["missense_or_plof"], config)
        elif variant_type == "spliceai_plof":
            config = _add_annotation_cols(
                ["plof_or_keep_SpliceAI", "SpliceAI_delta_score"], config
            )
            rare_threshold_config[
                "plof_or_keep_SpliceAI"
            ] = "plof_or_keep_SpliceAI == 1"

        elif variant_type == "spliceai":
            config = _add_annotation_cols(
                ["keep_SpliceAI", "SpliceAI_delta_score"], config
            )
            rare_threshold_config["keep_SpliceAI"] = "keep_SpliceAI == 1"

        elif variant_type == "deepripe":
            DEEPRIPE_COLS.append("keep_DeepRipe")
            config = _add_annotation_cols(DEEPRIPE_COLS, config)
            rare_threshold_config["keep_DeepRipe"] = "keep_DeepRipe == 1"
...
def run_association(
    ...
    if (
        ...
        & (test_type == "skat")
        & config["test_config"].get("aa_collapse_missense", False)
    ):
        variant_to_aa_map_file = config["test_config"]["variant_to_aa_map_file"]
        logger.info(
        ...
        logger.info(
            f"Number of variants with missing position: {exploded_annotations['Protein_position'].isna().sum()}"
        )
        logger.info("Replacing na positions with distinct dummies")
        pos_dummies = [
            f"dummy_{i}"
            for i in range(exploded_annotations["Protein_position"].isna().sum())
        ]
